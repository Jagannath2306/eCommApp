...
if (!process.env.CATEGORY_IMAGE_PATH) {
    throw new Error("CATEGORY_IMAGE_PATH is missing in .env");
}
const uploadFolderPath = path.join(__dirname, "../", process.env.CATEGORY_IMAGE_PATH);
...
const storage = multer.diskStorage({
    ...
    filename: async function (req, file, cb) {
        ...
        try {
            ...
            const exists = await Category.isExists(req.body.name, req.body.id);
            if (exists) {
